---
layout: cv
title: Antonino Ciancimino's CV
---
# Antonino Ciancimino
Studente al Politecnico di Milano

<div id="webaddress">
  <a href="mailto:antonino.ciancimino@proton.me">antonino.ciancimino@proton.me</a>
|
  <a href="https://github.com/Anto26">GitHub</a>
|
  <a href="https://www.linkedin.com/in/antonino-ciancimino-892852170/" >LinkedIn</a>
</div>

<br>

<img src="photo.jpg" width=200>

## Al momento

Studio al Politecnico di Milano, facendo la spola fra Milano e Palermo!

### Interessi

Elettronica, Elettrotecnica, Machine Learning, Linux (Fedora, Mint), Sviluppo Software (Electron, TensorFlow, DataTables), Sostenibilità, Etica.

## Istruzione e Formazione

`sett 2025 - presente` Milano, Italia<br>
 **Politecnico di Milano** <br>
 Laurea magistrale in Computer Science and Engineering <br>
 Traccia: Artificial Intelligence

`sett 2021 - mar 2025` Milano, Italia<br>
 **Politecnico di Milano** <br>
 Laurea breve in Ingegneria Informatica <br>
 Votazione: 108/110 <br>

`sett 2016 - lug 2021` Palermo, Italia<br>
 **ITST "Vittorio Emanuele III"** <br>
 Diploma Istituto Tecnico, Informatica <br>
 Votazione: 100/100 <br>
 Attività: Alternanza scuola-lavoro in imprenditorialità e start-up innovative.


## Esperienza lavorativa

 `set 2025 - mar 2026` Milano, Italia<br>
 **Supporto didattico agli studenti** presso Politecnico di Milano<br>
 Attività di supporto didattico agli studenti durante il laboratorio di Fondamenti di Informatica.

`set 2024 - mar 2026` Da remoto<br>
 **Insegnante privato** (Autonomo)<br>
 Insegnante privato di Inglese, Matematica e Informatica a studenti delle scuole superiori di secondo grado.

`lug 2020 - ago 2020` Donnalucata, Italia<br>
 **Junior Receptionist** presso Donnalucata Resort<br>
 Accoglienza, contabilità, teamwork e comunicazione.


## Progetti

`nov 2025 - dic 2025` **Perceived pain & Breast cancer classification**<br>
 *Artificial neural networks & Deep Learning Challenges*<br>
 Modelli di classificazione supervisionata: un ensemble di CNN 1D-bidirezionale LSTM per la predizione del dolore tramite time series, e reti neurali per la classificazione di immagini istologiche di cancro al seno. Stesura di report tecnici dettagliati.

`feb 2024 - giu 2024` **Board game GUI (Codex naturalis)**<br>
 *Software engineering project*<br>
 Implementazione client-server di un gioco da tavolo in Java e JavaFX, focalizzato su architettura OOP, UX e sviluppo agile. Sviluppata sia GUI che TUI.


## Licenze e Certificazioni

*   **Huawei Seeds For the Future** - Huawei (ott 2023)
*   **Version control with Git** - Politecnico di Milano (mag 2024)
*   **Stress Management** - Politecnico di Milano (giu 2023)
*   **Telling Science** - Politecnico di Milano (dic 2022)


## Competenze tecniche

*   **Altro:** Elettronica, Elettrotecnica, Telecomunicazioni.
*   **Linguaggi:** Java, C, C++, Python, JavaScript, PHP, VHDL, SQL.
*   **Intelligenza Artificiale e Dati:** PyTorch, TensorFlow, Machine Learning, Deep Learning, Reti Neurali, Numpy, Pandas, R.
*   **Sviluppo & Strumenti:** Linux (Fedora, Mint), Git, LaTeX, Electron.js, Node.js, SQLite, JDBC, MVC, JavaFX.


## Competenze linguistiche

`Italiano` Madrelingua

`Inglese` Scritto: C1, Parlato: C1

<!-- ### Footer -->
